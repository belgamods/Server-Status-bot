...
const USER_AGENT = `GrensLandRP bot ${require('./package.json').version} , Node ${process.version} (${process.platform}${process.arch})`;
...
exports.start = function(SETUP) {
  ...
  const sendOrUpdate = function(embed) {
    ...
    let dot = TICK_N % 2 === 0 ? 'GrensLandRP' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor(" GrensLandRP Status", "http://grenslandrp.be/assets/img/logo.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? ' 💻 GrensLandRP' : ' 👀 GrensLandRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHoe kan je de server joinen?','Je kan de server joinen doormiddel van "te wachten" in je F8 te typen.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Burgers in de stad:',`${STATUS}\n\u200b\n`);
 ...
 bot.on('ready',() => {
    ...
    bot.user.setActivity("\GrensLandRP", { type: "PLAYING" });
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Je bericht is succesvol gestuurd naar het staff-team van Amersfoort')
          .setTimestamp(new Date());
